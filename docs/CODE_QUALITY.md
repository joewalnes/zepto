...
**10. Multi-position document edits (multi-cursor, column-select) must use the shared `_adjust_pos_for_edit`/`_apply_edit_and_fixup_positions` helper — do not hand-roll a "fix up the other cursors" loop**
Any function that applies an edit at N tracked positions in the same document (multi-cursor insert/backspace, column-select insert/backspace/delete) must, after each individual position's edit, correctly re-derive every OTHER tracked position's `{line, col}` (and `{anchor_line, anchor_col}` if it has an in-flight selection). This is genuinely easy to get subtly wrong: a same-line edit only needs a column delta, but a line-join or a deletion spanning multiple lines needs BOTH the line and column adjusted together, and a naive "if delta != 0" guard (keyed off a same-line-only column delta) silently skips the line-changing cases entirely. That exact gap shipped in 4 of 5 independently-hand-rolled copies of this logic and caused real, silent cursor-position corruption — not a rendering glitch, an actual "your next keystroke edits the wrong part of the file" bug (bugs.md P1, fixed 2026-08-30; `_adjust_pos_for_edit`/`_fixup_other_positions`/`_apply_edit_and_fixup_positions` in `Editor.pm`, right before the column-editing helpers section). The shared helper expresses every edit as one small descriptor (`{start_line, start_col, end_line, end_col, ins_len}` — "delete this range, then insert this many newline-free characters at its start") and handles all three cases (same-line shift, line-join, multi-line deletion) as one general algorithm, so there is no separate line-join code path to forget. **If you add a 6th multi-position editing function** (e.g. a future column-select "insert newline" or a multi-cursor "duplicate line"), build it around `_apply_edit_and_fixup_positions` from the start rather than writing a new reverse-iterate-and-fixup loop — the column-select trio's history here (nobody separately noticed it needed a fixup loop at all, because rectangular edits never cross a row) is exactly why this was worth unifying instead of "just fixing the two multi-cursor functions." `tests/multi_cursor.t`'s `QA-REG-203`/`QA-REG-204` subtests (line-join and multi-line-selection-delete with a bystander cursor) are the regression net for the line-changing cases; extend those first if you touch this mechanism. `tests/column_select.t` (41 subtests) is the regression net proving the column trio's narrower, single-row-confined behavior is unchanged.
