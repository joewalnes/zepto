...
- File tree with many changed files (git status) and many collapsed folders now updates faster — VCS status indicators (the colored folder names showing "has changes inside") are now computed with a single pass over the changed-files list instead of rescanning it once per collapsed folder. No visible behavior change, just faster refresh on large repos with lots of uncommitted changes
...
- The tab bar's close/switch-tabs/quit hint now matches the bottom status bar's pill styling — rounded chip shapes with Title Case labels ("Close", "Tabs", "Quit") instead of plain lowercase text, with no stray underline and caps that match the pill's own color. Same information, same keyboard shortcuts, just styled consistently with every other pill in the UI. At very narrow terminal widths the hint now needs a little more room to appear than before (it degrades to blank rather than showing anything corrupted or truncated) — it's always reachable via the Commands palette either way
- Fixed the minimap's git-change indicators recomputing across the whole file on every single keystroke instead of only when the file's git status actually changed — on a large file (100k+ lines) this could add a noticeable delay to typing. The minimap now samples a bounded number of lines per row (like it already does for the text-density pattern) instead of scanning every line, so the cost no longer grows with file size. No visible change for typical files; an isolated single-line git change deep inside a very large, mostly-unchanged file may very rarely not show its own colored marker in the minimap (multi-line changes are unaffected)
- Hardened syntax highlighting against a pathological language grammar hanging the editor: each language's tokenizing step is now time-bounded, the same protection Find already had for search patterns. No shipped language grammar is known to have this problem today — this closes the gap so a future one couldn't freeze the editor either. If it ever did trigger, the affected line would just display unhighlighted rather than freezing anything. No visible change for normal files
