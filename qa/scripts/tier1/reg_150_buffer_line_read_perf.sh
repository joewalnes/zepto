...
# completion candidate. Neighboring (non-edited) rows and the status
# bar's cursor line:col were never observed to be affected by either
# bug, so those are still asserted directly on screen below.
