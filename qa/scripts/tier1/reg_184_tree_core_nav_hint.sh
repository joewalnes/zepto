...
# Fix: Renderer.pm::_core_nav_hint_text() is a single shared helper used by
# both _render_tab_bar (DOCUMENT context) and _render_context_status_bar
# (FILE_TREE context), so the wording can't drift between the two contexts
# the way it did before core_nav => 1 existed. This test asserts the
# FILE_TREE row renders the *identical* wording DOCUMENT context uses.
