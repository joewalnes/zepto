...
### P3: `qa/scripts/tier1/sess_009_toggle_pref.sh` is stale — expects the reverted full-block tab glyph
Found incidentally while triaging a full `make qa` run for an unrelated security-hardening change (bugs.md "Scorecard audit round 2" P2 items). `QA-SESS-009`'s "with the pref off, bare relaunch does not restore a.txt" assertion (`sess_009_toggle_pref.sh:33`) waits for the pattern `"█ \[untitled\] ⌥1"` — the full-block tab-cap glyph introduced by commit `1ae930b` ("Redesign tab bar to actually read as tabs"). That redesign was later reverted by commit `9689309` ("Revert tab caps from full-block to ◢/◣ triangles — root cause was a hangon bug"), which restored the `◢`/`◣` diagonal-triangle glyphs in `Renderer.pm` but never updated this QA script back to match — it now fails consistently (confirmed 2/2 standalone runs, not a concurrency flake) because the screen genuinely shows `◢ [untitled] ⌥1 ×◣`, not `█ [untitled] ⌥1`. `qa_assert_not_screen "█ a\.txt ⌥"` on the next line is unaffected (it's checking for absence, and `█` is absent either way, just for the wrong reason). Confirmed unrelated to any code touched by this session's changes (ImageConverter.pm/StateStore.pm/VCS/Git.pm) — pure tab-bar rendering staleness. Fix: update the pattern back to `"◢ \[untitled\] ⌥1"` (or a glyph-agnostic pattern) to match current `Renderer.pm` output.
