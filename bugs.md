...
### P2: Hover highlight paints one event late
Discovered while verifying the QA-REG-101 fix, and confirmed present on the pre-fix build too (not a regression from that fix). When a single discrete hover-motion event enters/leaves a status bar pill, the render that follows paints the *previous* hover state: hovering a pill shows it unhighlighted, and the next event (e.g. moving off the pill) paints it highlighted. With a real mouse the constant stream of motion events masks this (each event flushes the previous state), but the state machine is one event behind. Repro: inject `ESC[<35;X;YM` over a pill, screenshot, inject motion elsewhere, screenshot — highlights are inverted. Likely a stale snapshot of hover state somewhere between `_handle_mouse_hover()` and the status bar paint.
...
### P2: A few QA scripts are flaky under full-suite parallel load
`edit_020_bracket_pair` failed in 3/3 full `make qa` runs but passes standalone and in the `--filter edit_` subset; `ms_012_drag_tree_border` and `clip_009_paste_replaces` failed in some full runs only. `ms_012` dies silently (exit 1, no FAIL line): under `set -e`, its measurement pipeline (`grep ... | head | grep -oE`) kills the script when a grep matches nothing after a slow render. None of them use `qa_project`/`qa_git_repo`, and they predate the 2026-08-28 input/render fixes (failures show stale-render-free screens). Likely cause: fixed `sleep`-based waits starved by ~10 parallel hangon/tmux sessions. Candidates for `qa_expect`-based waits instead of sleeps.
