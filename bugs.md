...
### P3: 5 confirmed-dead subs — FIXED (2026-08-30)
...
**Fix**: Re-verified all 5 dead-code claims independently via `grep -rn` across `lib/`, `tests/`, `qa/`, `build.pl` before touching anything.
- `Renderer::_render_pill` deleted. Confirmed its nerd-font/non-nerd-font branches were byte-for-byte identical (both just push `$theme->color($bg_key) . $theme->color($fg_key)` then `" $text "`) — nothing worth salvaging into `_render_pill_list`.
- `Terminal::set_cursor_color` — **kept, and wired up** instead of deleted: `Editor/Commands.pm`'s `_apply_theme_pref` now calls `$self->{terminal}->set_cursor_color($cursor_color)` instead of duplicating the raw `print STDOUT "\x1b]12;...\x1b\\"; STDOUT->flush()` sequence. Verified `$self->{terminal}` is populated in every test/runtime construction path and that `Terminal::write()` (which `set_cursor_color` calls) does an unbuffered `syswrite` to the same `out_fh` (defaults to `\*STDOUT`), so this is behavior-preserving, not just a rename. Verified interactively via `hangon`: `⌃T` theme toggle still works with no errors/hang.
- `Terminal::read_available` deleted (dead; `read_blocking` is the only reader used anywhere). `use IO::Select` retained — still used by `read_blocking`.
- `Buffer::_debug_state` deleted.
- `Preferences::visual_width` — **left alone, not deleted.** Checked whether a concurrent agent's `tab_width == 0` divide-by-zero fix had landed on `main` yet (`git diff main -- lib/Zepto/Preferences.pm` was empty at the time of this fix) — it hadn't. Per the ambiguity noted in the original finding, deleting it here risked colliding with that other in-flight fix, and it's genuinely low-priority either way, so this one item is explicitly skipped/deferred rather than guessed at. Still dead in production as of this commit; `tests/preferences.t` is its only caller.

### P3: `AIComplete.pm::_parse_url` is dead code — FIXED (2026-08-30)
Referenced in the "Scorecard audit round 2" security-audit findings (item D) as never called; the actual request URL is built via naive string concatenation elsewhere in the same file instead.

**Fix**: Confirmed via `grep -rn '_parse_url' lib/ tests/ qa/ build.pl` that its only appearance anywhere in the repo was its own `sub _parse_url { ... }` definition (`AIComplete.pm:403-417`) — no callers, no tests. Deleted. `AIComplete.pm` has no test file (`tests/*.t`) at all currently (tracked separately as its own P2 finding — "the one network-calling module has zero test coverage" — not addressed here, out of scope for this cleanup), so there was no test coverage to lose. `make check`/`make build`/`make test` all clean after deletion.

### P3: `do_find_next`/`do_find_prev` are 77 lines of dead production code, confusingly named next to the real commands — FIXED (2026-08-30)
...
**Fix**: Confirmed via `grep -rn` that `CommandRegistry.pm` and every runtime dispatch path (`Editor.pm`) reference only `cmd_find_next`/`cmd_find_prev`, never `do_find_next`/`do_find_prev`; the latter's only references anywhere in the repo were their own definitions and the 8 call sites in `tests/editor.t`. Deleted both dead methods. Migrated the 3 subtests that used them ('Find next', 'Find not found', 'Find prev with selection finds previous match') to call the real `cmd_find_next`/`cmd_find_prev` instead — but their assertions could **not** just be transplanted unchanged: the deleted dead code did a simple continuous `index()`/`rindex()` scan and set a "Found"/"Not found" status message, while the real commands call `enter_find_mode()` (which re-anchors to the match nearest the cursor via FindEngine's `_find_nearest_match` on *every* call) before stepping via `_find_navigate()`, and never set a status message. Re-derived every expected position/selection by driving the real commands directly (a throwaway probe script) rather than guessing, and rewrote the subtests' assertions to match verified real behavior — see `tests/editor.t`'s updated 'Find next'/'Find not found'/'Find prev selects a match' subtests and their comments. Added a 4th subtest ('Find next falls back to cmd_find when no search term is set') covering the `else` branch that was previously untested. No coverage was silently lost: the deeper multi-match/wrap-around navigation logic (`_find_navigate`) already has thorough, dedicated coverage in `tests/find.t` ('Navigate to next match', 'Navigate wraps at end', 'Navigate wraps at start', 'Navigate with no matches') — that coverage was untouched. Verified via `prove -Ilib tests/editor.t`: 144 tests, all pass, zero noise. Also added `qa/scripts/tier1/reg_196_find_next_prev_real_cmd.sh` (`QA-REG-196`, see `qa/40_regression_bugs.txt`) — the real `cmd_find_next`/`cmd_find_prev` (Ctrl+J/Ctrl+K) had **zero** interactive/QA coverage before this fix, since the dead code was the only place anything resembling "find next/prev" got exercised by name. Verified the new QA script is non-tautological by temporarily neutering `cmd_find_next` and confirming the script correctly fails (2 of 6 checks) before restoring it.

**Behavioral discovery (not fixed here, logged separately)**: while deriving the real expected test values, found that `cmd_find_next`/`cmd_find_prev` re-entering `enter_find_mode()` on every single call (rather than just stepping an already-open find session) can cause repeated Ctrl+J/Ctrl+K presses to "get stuck" on the same match in some cursor positions, or skip over a match entirely, because the nearest-match re-anchor and the `_find_navigate` step compound unpredictably depending on exactly where the cursor landed from the previous call. See new "Found while fixing the dead-code cleanup (2026-08-30)" section below for the full writeup — flagged as a new bug, not fixed as part of this cleanup (out of scope).
...
### P3: Minor inconsistency — one `eval` swallow is documented as intentional, a structurally identical one isn't — FIXED (2026-08-30)
...
**Fix**: Added the matching `# Ignore callback errors` comment to `StateStore.pm:171`'s `eval`, mirroring `Preferences.pm:210` exactly. For the `disable_raw_mode`/`enable_raw_mode` `warn` asymmetry: checked `docs/CODE_QUALITY.md`'s "Global destruction order" pitfall note ("In `DESTROY` and cleanup code, filehandles may already be closed"). Confirmed `disable_raw_mode`'s only caller is `Terminal::cleanup()`, which runs on every normal shutdown path (Ctrl+Q, signals, etc.) — i.e. this is a cleanup/shutdown path, not a startup path, and `in_fh` may legitimately already be gone by the time it runs. Adding a `warn` here would risk spraying stderr noise on ordinary exit (violating Rule 3's "no unexpected output on stdout/stderr"), whereas `enable_raw_mode`'s `warn` fires at startup where a failure is unambiguously abnormal and actionable. Decided the asymmetry is intentional and documented it in place with a comment on `disable_raw_mode` explaining why, rather than adding a `warn` — kept the change minimal/low-risk per the task's own guidance to prefer this when there's a plausible deliberate reason.

### P3: `⌃` (U+2303) control-key glyph hand-duplicated 13× in `Renderer.pm` — FIXED (2026-08-30)
...
**Fix**: Added `use constant CTRL_GLYPH => "\x{2303}";` to `Renderer.pm`, next to the existing "Box-drawing characters" constant block (not `Zepto::Chars`, since that module is specifically for Nerd Font/ASCII fallback *pairs* — `⌃` is a plain Unicode glyph used unconditionally regardless of `Zepto::Chars->enabled()`, so it doesn't fit that abstraction). Replaced all 13 inline-literal sites (line numbers had shifted slightly from concurrent work; found via fresh `grep -n 'x{2303}'`) with `CTRL_GLYPH . "..."` concatenation. Verified byte-identical output before/after two ways: (1) a full `diff` of old vs. new `Renderer.pm` confirmed every changed line was a mechanical `"\x{2303}X"` → `CTRL_GLYPH . "X"` swap with no other changes; (2) a throwaway harness loaded both the pre-change and post-change `Renderer.pm` in separate processes and called `_core_nav_hint_text()` (which uses `CTRL_GLYPH` in both its labeled and compact branches) across several width values covering the undef/compact/labeled branches — output was byte-identical. Also verified interactively via `hangon`: status bar hints (`⌃W close`, `⌃G`, `⌃␣`), command palette title (`⌃␣ Commands`), and palette shortcut column (`⌃N`, `⌃O`, `⌃S`, etc.) all render correctly.

### P3: Tab Width's `1`/`16` bounds are duplicated as bare literals — FIXED (2026-08-30)
...
**Fix**: Added `use constant { TAB_WIDTH_MIN => 1, TAB_WIDTH_MAX => 16 };` directly above `cmd_set_tab_width` in `Editor/Commands.pm` (line numbers had shifted to ~1285 from the cited 1374/1380 due to the `do_find_next`/`do_find_prev` deletion earlier in the same file). Used the constants in all three places that previously hardcoded the bounds: the `hint => '1-16'` string (now `TAB_WIDTH_MIN . '-' . TAB_WIDTH_MAX`), the `$input >= 1 && $input <= 16` validation, and the `"Invalid tab width. Use a number 1-16."` error message (a third hardcoded-bounds site the original finding didn't call out, found while making the fix). Verified byte-identical output (`perl -e` one-liner printing both interpolated strings against the new constants matches the original literal text exactly) and verified interactively via `hangon`: the footer input shows "(1-16)", an out-of-range value (99) shows "Invalid tab width. Use a number 1-16.", and a valid value (8) is accepted and applied.
...
## Found while fixing the "Scorecard audit round 2" dead-code/consistency items (2026-08-30)

### P3: Repeated Ctrl+J/Ctrl+K (Find Next/Prev) can get "stuck" on the same match or skip one entirely
Discovered while re-deriving real expected behavior for `cmd_find_next`/`cmd_find_prev` to migrate `tests/editor.t`'s dead-code-only find tests (see "`do_find_next`/`do_find_prev`..." above) and while building `qa/scripts/tier1/reg_196_find_next_prev_real_cmd.sh`.

`cmd_find_next`/`cmd_find_prev` (`Editor/Commands.pm:696-722`) both call `enter_find_mode()` — which re-runs `_update_find_matches()` → `_find_nearest_match()` and jumps the cursor to whatever match is nearest the *current* cursor position — **on every single invocation**, then call `_find_navigate($direction)` to step one further from there. This is correct and predictable when starting from a cursor position that isn't sitting exactly on a previous match (verified: from a clean `Ctrl+G`-reset position, both directions land exactly where expected). But `_jump_to_match()` (`Editor.pm:3080-3100`) always leaves the cursor at the **end** of the just-selected match (`col + length`), and `_find_nearest_match`'s "prefer match at-or-after cursor" comparison (`Editor.pm:3014-3049`) uses the match's own *start* column against that end-of-match cursor position — so immediately re-invoking the same command (or the opposite one) from that landing spot re-anchors somewhere other than a naive "current index ± 1" would predict. Concretely reproduced via `hangon` on a 3-match file (`MARKER one` / `line two` / `MARKER three` / `line four` / `MARKER five`): pressing Ctrl+J repeatedly (with a dismiss in between, matching real usage) cycles between only 2 of the 3 matches (line 1 and line 5), **never landing on line 3's match at all**; and pressing Ctrl+K immediately after a Ctrl+J that landed on the very first match in the file visibly does nothing (cursor doesn't move) because the re-anchor-then-step-back cancels out. Not fixed here — out of scope for the dead-code cleanup task that surfaced it. `qa/scripts/tier1/reg_196_find_next_prev_real_cmd.sh` deliberately resets the cursor via Ctrl+G before each check specifically to avoid asserting on this quirky chained behavior (see the script's own comment), so it does not regression-guard this specific issue — a future fix here should add its own dedicated QA script reproducing the stuck/skip sequence once someone decides what the *intended* behavior should be (a simpler design that just uses `find_current` as-is without re-deriving "nearest to cursor" on every call would probably not have this problem, but changes user-visible behavior and needs a product decision, not a unilateral fix).
