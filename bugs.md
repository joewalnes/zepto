...
### ~~P1: Discoverability Contract gaps found by manual visual sweep (2026-08-30, not yet fixed)~~ PARTIALLY FIXED (2026-08-30)
...
**Still open (not touched by this fix, tracked as debt):** finding #3, the FILE_TREE context still has no hint for quit, focus-switch back to the editor, or tab navigation while the tree is focused — out of scope for this pass, which was scoped to the DOCUMENT-context tab bar only.
...
### P3: `editor_bg` — stale/nonexistent theme role name in the Markdown inline-image spacer-row path

Found via code audit while investigating the light-theme background-fill report above (see that entry for the negative result on the actual reported bug). `Renderer.pm:2187`, inside `_render_text_area`'s handling of blank spacer rows reserved for inline Markdown image placement (`$entry->{type} eq 'image_spacer'`), reads `my $bg = $theme->color('editor_bg');`. No theme in `Theme.pm` defines an `editor_bg` role (the correct/only role for the main text-area background is `bg`) — `Theme::color()` returns `''` for an unknown role (`$self->{colors}{$role} // ''`), so this line silently emits *no* background-color escape code at all for the spacer row's text-area fill, in both themes. Unlike the false-positive above, this is a genuine latent defect matching the task's own hypothesis (a) — just in a different, narrower code path (Markdown files with inline images, kitty-graphics-capable terminals only, on spacer rows specifically) than the one reported. Not otherwise user-visible in most terminals/themes tested so far (terminal default background often coincidentally reads as acceptable, the same masking effect described above), but is a real bug in its own right and should use `$theme->color('bg')` like every other text-area fill site in the file. Not fixed here (out of scope — this was a drive-by audit finding while chasing a different, unconfirmed report, not the assigned task); flagging for a dedicated fix + `tests/renderer.t` regression assertion + QA script covering a Markdown file with an inline image in light theme.
