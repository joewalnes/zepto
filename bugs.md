...
### P3: Theme toggle pill icon doesn't switch with theme
Found while reworking the status bar. `toggle_theme`'s `CommandRegistry` entry has `icon => 'theme_dark'` with a comment `# dynamic: theme_dark or theme_light`, but nothing in `Renderer.pm` actually swaps it — the pill always shows the "moon" icon even in light theme. Pre-existing, unrelated to the status bar rework; out of scope to fix here.
