...
**Follow-up:** the revert itself introduced a regression — 6 `sess_*` session-restore QA scripts and 2 mouse-click scripts (`ms_009`/`ms_010`) plus `reg_182` had hardcoded the full-block `█` as a tab-bar-vs-file-tree-sidebar disambiguation anchor for their screen assertions, so they broke the moment `█` reverted back to `◢`/`◣`. Caught via a full post-revert `make qa` run, fixed via targeted codepoint substitution (`◢` for `█`) across all 9 scripts, verified via targeted reruns plus full `make test`/`make check`. Committed separately as `fd218d9`.
...
### ~~P1: `cmd_transform` (⌥T, "Transform via Shell") can deadlock the whole editor~~ FIXED
...
**Root cause:** confirmed exactly as described — `cmd_transform`'s `eval` block did `$output = <$out_fh>;` (a full blocking slurp of stdout to EOF) followed only afterward by `$stderr_text = <$err_fh>;`. Reproduced first with a standalone repro (`perl -e 'print STDOUT "x"x100; print STDERR "e"x100000; print STDOUT "y"x100'` piped through the old code path) and via a real interactive `hangon` session: the whole editor froze past the ~64KB stderr pipe-buffer threshold, with no Ctrl-C recovery, exactly as reported.

**Fix:** replaced the two sequential blocking reads with an `IO::Select`-driven loop that alternates `sysread()` on whichever of stdout/stderr is actually ready (`$sel->can_read`), removing each handle from the select set on EOF, so neither pipe can ever back up while the other is starved. `$output`/`$stderr_text` now accumulate via string concatenation across possibly-many `sysread` calls instead of one `<$fh>` slurp each. Added `use IO::Select;` alongside the existing `use IPC::Open3;`.

**Tests:** `tests/transform_deadlock.t` — calls `cmd_transform`'s `on_submit` directly with the exact stderr-flooding shell command, wrapped in an outer `alarm(8)` safety net (needed because `cmd_transform`'s own `eval` swallows an alarm-interrupted blocked read as a generic "Transform failed" and returns normally either way — so the real signal is elapsed wall-clock time, not exception propagation). Confirmed this test fails correctly against the pre-fix code first (blocked for the full 8s alarm bound, wrong/unchanged document content) before making the fix, then passes in ~0s after.

**Interactive verification (`hangon`):** ran the exact deadlock shell command live via the real `⌥T` → "Transform via Shell" flow — completed instantly (buffer replaced with the expected `x`×100 + `y`×100), no freeze. Also verified normal transform usage (`tr a-z A-Z`) still works correctly with the new concurrent-read implementation, confirming the rewrite didn't regress the common case.

**QA:** `qa/scripts/tier1/reg_187_transform_shell_deadlock.sh` (`QA-REG-187`) — runs the real binary end-to-end via the palette, sends the stderr-flooding command, and polls (bounded, not a fixed sleep) for the transformed output to appear.

### ~~P1: Clipboard paste has no timeout — a hung paste command freezes the UI indefinitely~~ FIXED
...
**Root cause:** confirmed — `paste_from_clipboard` did `my $text = do { local $/; <$fh> };` with no alarm guard at all. Reproduced first with a standalone repro script (a bare blocking slurp against a `sleep 3` child, no alarm) that blocked for the full duration with no way to recover before the child exits, matching the reported failure mode exactly.

**Fix:** added `CLIPBOARD_PASTE_ALARM_SECS => 3` and wrapped the blocking slurp in the same `local $SIG{ALRM} = sub { die ... }; alarm(N); ...; alarm(0);` idiom `FindEngine.pm::_match_with_alarm` established, including the `alarm(0)` safety net outside the `eval` so no alarm is ever left armed. On timeout: `kill('TERM', $pid)` + `waitpid($pid, 0)` to avoid leaving a zombie, then returns `undef` — a return value distinct from `''` (empty clipboard), so callers can tell "nothing to paste" apart from "the read timed out." `cmd_paste` (`Editor/Commands.pm`) now checks `!defined $text` first and calls `show_error_message(_user_error("Paste failed", "clipboard read timed out"))` before falling through to the empty/non-empty-text branches — the existing `_user_error()`/`show_error_message()` pattern, no new error-reporting mechanism introduced.

**Tests:** `tests/terminal.t` "paste_from_clipboard times out on a hung clipboard command" — overrides `$term->{_clipboard_paste_cmd} = ['sleep', '30']` (a real hang, portable, no macOS/Linux-specific flags) and asserts the call returns `undef` and completes in well under 30s (bounded by the real 3s alarm plus margin) — a genuine real-timing assertion, not a mocked timer.

**Interactive verification (`hangon`):** confirmed normal `⌃V` paste still works correctly post-fix (status bar showed "Pasted", pasted text landed correctly) — sanity check that the added `!defined $text` branch didn't regress the common path. The actual hang-recovery scenario is verified end-to-end (real binary, real Ctrl+V, real hung clipboard command) by the QA script below.

**QA:** `qa/scripts/tier1/reg_188_clipboard_paste_timeout.sh` (`QA-REG-188`) — shadows every clipboard-paste command `Terminal.pm::_detect_clipboard_commands` might pick (`pbpaste`, `xclip -o`, `xsel --output`, `wl-paste`) with fakes that sleep 30s, placed earlier in `PATH` than the real tools (confirmed `PATH` is inherited by freshly-started `hangon` sessions), presses a real `⌃V` in the compiled binary, and asserts the "Paste failed" message appears within 8s and the editor remains responsive afterward (typed text lands normally).

### ~~P1: "Tab Width" preference has no effect on rendering existing tab characters~~ FIXED
...
**Root cause:** confirmed exactly as described, plus one additional gap found while fixing it: `Editor.pm`'s WrapMap-rebuild condition (`render()`) only rebuilt the `WrapMap` object when the *viewport width* changed (`$wm->{width} != $wrap_width`) — a `tab_width` preference change alone (same viewport width) left the existing `WrapMap` object in place with its stale `tab_width`, so even after threading the width through `WrapMap`/`Renderer` correctly, the palette command would have had zero *visible* effect until an unrelated resize or edit happened to trigger a rebuild. Confirmed via a live `hangon` repro before fixing: setting Tab Width to 2 on an already-open tab-indented file left the rendering unchanged.

**Fix:**
- `Renderer.pm`: `_expand_tabs`/`_char_to_visual_col`/`visual_to_char_col` now accept an optional trailing `$tab_width` argument (falls back to a package-level effective width `$_tab_width`, which itself falls back to `TAB_WIDTH` if unset/invalid — so every existing call site with no width argument keeps working unchanged). Added `set_tab_width($width)`, following the exact same "package-scoped `my` variable + `set_*` class method" pattern `Zepto::Chars::set_enabled`/`$_nerd_font_enabled` already established for a different per-render preference (nerd_font) — chosen over threading an explicit width parameter through all ~14 in-module call sites because it's the codebase's own existing precedent for "per-render-pass config synced from prefs," not a new pattern. `render()` now calls `$class->set_tab_width($prefs ? $prefs->tab_width() : undef)` right next to the existing "Sync Chars module with prefs" line.
- `WrapMap.pm`: its 4 call sites into `Zepto::Renderer::_expand_tabs`/`_char_to_visual_col`/`visual_to_char_col` now explicitly pass `$self->{tab_width}` (the value it already stored but never read) as the trailing argument, rather than relying on Renderer's package-level default — more robust than relying on render() having run first, since `wrap_line()`/`visual_to_doc()` etc. can be called independently of a full render pass.
- `Editor.pm`: the WrapMap-rebuild condition now also checks `$wm->{tab_width} != $self->{prefs}->tab_width()`, so a tab_width preference change alone triggers a rebuild (previously only a viewport-width change did).
- `Editor.pm`'s two mouse-click `visual_to_char_col` call sites (click-to-position, drag-select) now pass `$self->{prefs}->tab_width()` explicitly too, for the same "don't depend on a render having already synced the package default" robustness reason.

**Tests:** `tests/renderer.t` — three new subtests: explicit-width-argument correctness for all three helper functions (width 4 vs 8, would be identical under the old hardcoded constant), `set_tab_width()`'s package-level default behavior including its `<1` fallback to `TAB_WIDTH`, and an end-to-end `render()`-with-`$prefs` sync check (tab_width 2 then 8, confirms the helpers pick up each). `tests/wrapmap.t` unaffected/still passing (existing coverage). Confirmed the new subtests fail against the pre-fix code first (temporarily reverted the `render()` sync call and reran — `_expand_tabs`/etc. had no width parameter to even pass, and the width-2-vs-8 comparison showed identical output).

**Interactive verification (`hangon`, Rule 2, required for this one specifically):** opened a tab-indented test file (default Tab Width 4, tabs visibly at 4-col stops), used the command palette to set Tab Width to 2 — the file's existing tab-indented lines re-wrapped/re-rendered immediately at the new width (no edit, no resize, no reload needed). Repeated for Tab Width 8 — re-rendered again at the wider stop. Confirmed via a marker-column measurement (not just "screen changed"): a `MARKERTAB` token right after a leading tab moved from screen column 9 (width 2) to column 15 (width 8), an exact 6-column delta matching (8-2). Also reproduced the *bug* first, live: with the `render()` sync call temporarily reverted, the same steps left the marker's column completely unchanged across the Tab Width 2→8 change, confirming this is a real, user-visible fix, not a no-op.

**QA:** `qa/scripts/tier1/reg_189_tab_width_rerender.sh` (`QA-REG-189`) — the marker-column measurement above, run against the real compiled binary; confirmed to fail (delta 0, not 6) against the pre-fix code before this fix and pass after. Cross-referenced from `QA-PREF-007` (`qa/36_preferences.txt`, tab width on insertion) via `qa/40_regression_bugs.txt`.

### ~~P3: `Preferences::visual_width` divides by zero on `tab_width == 0`~~ FIXED
See "P3: 5 confirmed-dead subs" below for the original combined finding — split out here since it got its own root-cause/fix/test writeup. `Preferences.pm:310` — `Preferences::set()` performed zero validation on any preference value. A hand-edited `tab_width: 0` in `preferences.json` (which is intentionally hand-editable per this project's docs) causes an uncaught fatal on the next `visual_width()` call.

**Root cause:** confirmed — `set()` wrote `$self->{prefs}{$key} = $value` with no validation of any kind. Additionally, while fixing this, found that `set()`-only validation would NOT have actually closed the gap the bug describes: `_load_from_store()` (the startup-load-from-disk path — the literal scenario of a hand-edited `preferences.json`) writes directly into `$self->{prefs}{$key}`, bypassing `set()` entirely, and `_apply_external_changes()` (cross-instance sync via `StateStore`'s `on_change`) does the same. A test written against `set()`-only validation would have passed while the actual reported scenario (hand-edited file, loaded at startup) remained completely unprotected — caught by writing the load-path regression test first and confirming it failed.

**Fix:** extracted a shared `_validate_value($key, $value)` function (for `tab_width`: clamps to `1` if undefined, non-numeric, or `< 1`) and call it from all three paths that can populate `$self->{prefs}{tab_width}`: `set()`, `_load_from_store()`, and `_apply_external_changes()`. Clamps rather than rejecting outright, so a bogus edit degrades gracefully instead of silently discarding the whole preference write. As a second layer of defense, `visual_width()` itself also now clamps its own `$tab_width` (whether from `$self->tab_width()` or an explicit override argument) to `>= 1` before using it in a modulus — belt-and-suspenders in case some future caller ever bypasses both `set()` and load, e.g. by passing an explicit override.

**Tests:** `tests/preferences.t` — `set('tab_width', 0)`/`(-5)`/`('not_a_number')` all clamp to `1`; a valid positive value still sets normally; `visual_width()` after a clamped `set()` doesn't die. A separate subtest specifically covers the load-path gap: seeds `tab_width => 0` directly via `$store->put('preferences', {...})` (not via `set()`), constructs a fresh `Preferences->new(state_store => $store)`, and confirms the loaded value is `1`, not `0` — confirmed this subtest fails against the pre-fix `_load_from_store()` (returns `0`, unclamped) before wiring `_validate_value()` into that path, and passes after. A third subtest covers `visual_width()`'s own defensive guard directly with explicit `0`/negative override arguments.

**QA:** `qa/scripts/tier1/reg_190_tab_width_zero_clamp.sh` (`QA-REG-190`) — pre-seeds `{"tab_width": 0}` directly into `$state_dir/preferences.json` (simulating a hand edit, not a `set()` call), launches the real compiled binary, confirms it starts up without crashing, and confirms the "Tab Width" palette command's prefilled value shows the clamped `1`, not `0`.

### ~~P1: `qa/scripts/tier1/sec_001_path_traversal.sh` cannot fail under any outcome~~ FIXED
Confirmed the most severe of the tautological-test findings below because it's tagged security: the script typed a path (`/tmp/../tmp/zepto_qa_sec001_safe.txt`, which normalizes right back into `/tmp` — not a real traversal-outside-root attempt) and had **no `qa_fail` branch anywhere** — both the success and the "possibly rejected" fallback branch called `qa_pass`.

**Investigation:** Before rewriting, checked what root-confinement Save-As actually has. `Editor/Commands.pm::cmd_save_as`/`_finish_save_as` write to whatever path the user types with zero root confinement — by design, like every desktop editor's Save As; there is no "sandbox" for it to escape, so a traversal test against it can never be meaningful. The real, documented path-traversal protection in this app is `FileTree.pm`'s `_path_within_root()` (`Cwd::realpath()` + `index($real, "$root/") == 0`, audited in `docs/SECURITY.md`'s P3 symlink-traversal entry). It's implemented independently in two places: `_scan_dir_one_level()` (interactive tree widget, already covered by `QA-SEC-003`/`sec_003_symlink.sh`) and `_walk_for_files()` (the Ctrl+O "Open File" fuzzy picker's file list, via `_filter_all_files()` in `Editor/Palette.pm` — not covered by any existing script).

**Fix:** Rewrote the script to target the uncovered code path. Sandbox: a `qa_project` temp dir with a real file plus a symlink pointing to a separate outside directory containing a "secret" file. Open Ctrl+O with no typed query (so the picker's own screen rendering is the only source of the assertions — nothing typed can leak into them) and assert the in-project file IS listed (sanity: catches a broken/no-op picker) while the file only reachable through the escaping symlink is NOT listed.

**Verified non-tautological (Rule 5):** disabled the `next if -l $full && !$self->_path_within_root($full)` guard in `FileTree.pm::_walk_for_files` (`0 &&` prefix), rebuilt — script correctly failed (`secret_outside_sec001.txt` appeared in the picker via `escape_link_sec001/`). Restored the guard, rebuilt — passes again. Ran standalone 3x clean.

**Note:** `qa/38_security.txt`'s own `QA-SEC-001` entry text ("No shell injection in VCS operations") doesn't match what this script (or the bugs.md audit finding) describes — a pre-existing ID/description drift in the QA docs, not introduced or fixed here. Flagged for a future pass rather than fixed in this session (out of scope — same drift pattern also found on `QA-NAV-010` and `QA-GUT-002` below).

### Security (P2)

### ~~P2: `ImageConverter.pm` uses a predictable temp filename with no exclusive creation — symlink-follow risk~~ FIXED
`ImageConverter.pm:76` built `"$tmpdir/zepto-img-$$-$basename"` via string concatenation — a predictable path in the shared `/tmp`, with no `O_EXCL`/exclusive creation — then ran `sips`/`convert` against it (`ImageConverter.pm:71-93`). Unlike `Document.pm`'s save path (which correctly uses `File::Temp` for exactly this reason), `sips`/`convert` would follow a pre-existing symlink at that predictable path. On a shared multi-user host, another local user who can observe the PID (`ps`) and the image's basename could pre-plant a symlink pointing at a file the victim can write (e.g. `~/.bashrc`), causing the victim's own image-preview invocation to overwrite that target through the symlink.

**Root cause confirmed real, not theoretical:** manually reproduced on macOS — planted a symlink at a predicted `zepto-img-$$-<basename>.png` path pointing at a victim file, ran `sips -s format png src.jpg --out <symlink>`, and the victim file's content was silently replaced with PNG bytes while the symlink itself stayed intact. `convert` (ImageMagick) exhibits the same behavior.

**Fix:** `ensure_png()` now uses `File::Temp::tempfile('zepto-img-XXXXXXXX', DIR => $tmpdir, SUFFIX => '.png', UNLINK => 0)` — unpredictable name, exclusive `O_EXCL`-backed creation — matching `Document.pm`'s existing atomic-save precedent exactly. Verified `SUFFIX => '.png'` doesn't change behavior: both `sips` and ImageMagick `convert` honor the explicit format flag (`-s format png`) / `PNG:` prefix regardless of the actual output filename's extension — confirmed empirically with real JPEG→PNG conversions at extensionless, `.tmp`-suffixed, and `.png`-suffixed output paths; `sips` only emits a (already-suppressed) stderr warning for a non-`.png` suffix, never a functional failure.

**Tests:** `tests/image_converter.t` — three subtests: normal conversion still produces a valid PNG; the output path no longer matches the old predictable `"zepto-img-$$-basename"` pattern (uses the new random-suffix pattern instead); and the direct attack repro — pre-plants a symlink at the exact old predictable path pointing at a victim file, runs a real conversion, and confirms the victim file is never touched. Confirmed test-before/fix/test-after: reverted the fix and re-ran — the naming and symlink-follow subtests both correctly failed (the symlink-follow subtest initially gave a false pass on the very first attempt because `ensure_png()`'s own per-source-file result cache short-circuited the second conversion call when reusing the same source file across subtests — fixed by using a fresh, uniquely-named source file for that subtest so the cache can't mask a regression); restored the fix and re-ran — all pass. QA: `QA-REG-193` in `qa/40_regression_bugs.txt`, script `qa/scripts/tier1/reg_193_image_converter_tempfile.sh` — opens a real JPEG needing conversion on a Kitty-graphics-capable terminal and confirms the full pipeline via the raw output stream (`hangon readall`, since headless hangon/tmux can't rasterize transmitted Kitty graphics pixels even via `hangon screenshot`): the Kitty APC image-transmission escape sequence is present with the correct format/id, and its base64 payload decodes to a real PNG. Verified interactively via `hangon`.

### ~~P2: AI API key briefly written world-readable before `chmod 0600` catches up~~ FIXED
`StateStore.pm:103-110` opened the secrets temp file with default umask permissions (often `0644`), wrote the AI API key content, closed it, and only *then* `chmod 0600`'d it before the atomic rename. A narrow but real window where the key was world-readable, contradicting `docs/SECURITY.md`'s parenthetical "secrets: ... (mode 0600)" claim, which implies secrets are always protected.

**Fix:** the `secrets` category now uses `sysopen($fh, $tmp_path, O_WRONLY|O_CREAT|O_EXCL, 0600)` — mode set atomically at creation, no window at all. Other categories (`preferences`, `history`) intentionally keep plain `open` at default umask permissions per this file's own category-docs comment ("preferences: ... user would sync across machines"; "history: ... accumulated state") — they hold no secrets and may be synced/managed like any other dotfile, so restricting them would be a behavior change with no security benefit. `O_EXCL` also closes the same symlink-follow risk class fixed in `ImageConverter.pm` above, since the temp path (`"$path.tmp.$$"`) is still PID-based/predictable.

**Tests:** checking only the *final* file's permissions would not catch this bug (both the old and new code converge on 0600 by the time `put()` returns), so `tests/state_store_secrets_race.t` instead intercepts the file-creation syscall itself: it overrides `CORE::GLOBAL::open`/`CORE::GLOBAL::sysopen` in a `BEGIN` block *before* `Zepto::StateStore` is compiled, and records the file's mode on disk immediately after creation — before any write/close/chmod/rename can run. This is deterministic, not timing-dependent, so it can't flake. (Lesson learned building this: a `CORE::GLOBAL::open`/`sysopen` override changes how bareword-filehandle `open` calls *parse*, not just their runtime behavior, for every module compiled afterward — the first attempt broke `Cwd.pm`'s internal bareword-FH usage with a "Bareword not allowed" compile error; fixed by preloading every one of `StateStore.pm`'s transitive dependencies — `File::Spec`/`Cwd`/`JSON::PP`/`File::Temp`/`Fcntl` — before installing the override, confining its effect to exactly the module under test.) Confirmed test-before/fix/test-after: reverted the fix and re-ran the race test — failed exactly as expected (observed the temp file at mode 0644, under a forced permissive `umask 0022`, at the moment of `open()`, well before the later `chmod` ran); restored the fix and re-ran — passes, and the final file is still confirmed mode 0600. QA: `QA-REG-194` in `qa/40_regression_bugs.txt`, script `qa/scripts/tier1/reg_194_secrets_file_mode.sh` — drives the real user-facing flow (Command Palette → "AI Completion: Setup", the only caller anywhere that writes the `secrets` category) end-to-end and confirms the resulting `secrets.json` is mode 0600. Verified interactively via `hangon`.

### ~~P2: `VCS/Git.pm::is_tracked()` has a git argument-injection edge case~~ FIXED
`VCS/Git.pm:103`: `_git('ls-files', '--error-unmatch', $rel_path)` had no `--` separator before `$rel_path`. A file whose relative path starts with `-` could be parsed by git as an option rather than a pathspec, causing `is_tracked()` to misreport (not itself exploitable for code execution/disclosure — the other `git show` calls at `Git.pm:121,150` are safe since they prefix with `HEAD:`/`:`, which can't start with `-`).

**Fix:** added `'--'` before `$rel_path`: `_git('ls-files', '--error-unmatch', '--', $rel_path)`. Audited every other `_git(...)` call site in `VCS/Git.pm` for the same gap: both `git show` calls (`"HEAD:$rel_path"`, `":$rel_path"`) remain safe since those prefixed forms can never start with `-`; `get_worktree_status`'s `git status --porcelain` takes no pathspec at all. Also checked `FileTree.pm`'s separate `git ls-files` calls (used for its fast file-list build) — these pass fixed flag-only argument lists with no user-controlled pathspec argument at all, so they were never at risk. No other gaps found anywhere in the codebase.

**Tests:** `tests/vcs.t` — two new subtests create a git repo with a `-weird.txt` file at the repo root (so its relative path genuinely starts with `-`, not just a subdirectory path), one tracked and one untracked, created via Perl `open()` with an explicit path (never shell globbing, which would otherwise treat the leading `-` as a flag). Confirmed test-before/fix/test-after: reverted the fix and re-ran — the tracked-file subtest correctly failed (misreported as untracked, since `-error-unmatch` and the bare filename together confused git's option parsing); restored the fix and re-ran — both pass. QA: `QA-REG-195` in `qa/40_regression_bugs.txt`, script `qa/scripts/tier1/reg_195_git_dash_filename.sh` — same revert/restore cycle confirmed the same result via the QA harness independently. This script is intentionally non-interactive (no hangon/zepto session): `grep -rn is_tracked lib/` confirms this method has **zero callers anywhere in the editor's runtime code today** (only exercised by unit tests) — there is no command, keybinding, or UI element to drive interactively, so Rule 2 (UI discoverability) does not apply to this particular fix. The script instead calls the real `Zepto::VCS::Git` library code directly against a real git repository, mirroring the interactive scripts' spirit as closely as that constraint allows.
...
### ~~P2: 6 more tautological QA scripts follow the exact anti-pattern `qa/README.md` documents and forbids~~ FIXED
Each had a dual-`qa_pass`-on-one-condition fallback (or, for `reg_070`, computed both sides of a comparison and never actually compared them) that let a real regression pass silently. Fixed all 6 (2 scripts share one bug, so 5 root-cause fixes below), each verified per Rule 5 by temporarily breaking the underlying behavior, confirming the rewritten script fails, then restoring and confirming it passes — 2-3 standalone runs (`perl qa/runner.pl --tier 1 --filter <name>`) after each restore with no flakiness (`prmt_006` needed one extra fix, see below).

- **`qa/scripts/tier1/tab_017_cache_theme.sh` (QA-TAB-017) and `qa/scripts/tier1/reg_084_tab_theme_cache.sh` (QA-REG-084)** — both `qa_pass`ed regardless of whether the tab bar actually re-colored after a theme toggle. Both guard the same underlying bug (`Renderer.pm`'s tab-bar cache key, `~line 900`, already includes `$theme->name()` — this just wasn't being tested), so rather than leave two identical tests or retire one (both IDs are cross-referenced from `bugs.md`/`QA-THM-004`), differentiated them by entry point and color property so they exercise genuinely different renders: `tab_017` drives the switch via the command palette ("Theme: Dark"/"Theme: Light", deterministic regardless of starting theme) and asserts the tab **bar's** background ANSI code (`48;2;30;32;44` dark vs `48;2;230;233;239` light); `reg_084` drives it via the raw ⌃T key (the original bug report's exact trigger) and asserts the **active tab's** background code (`48;2;52;79;138` dark vs `48;2;114;135;253` light), round-tripping dark→light→dark. Both capture raw ANSI via `tmux capture-pane -e` (same technique as `QA-REG-140`), not the color-stripped `hangon screen`/`qa_screen` text. Verified: removed `$theme->name()` from the `Renderer.pm` cache-key `join()` — both scripts correctly failed (stale dark color persisted after switching to light); restored — both pass.
- **`qa/scripts/tier1/nav_010_scroll_no_move.sh` (QA-NAV-010)** — named to verify Ctrl+Up/Down scrolls *without* moving the cursor, but `qa_pass`ed even when the cursor moved. **Investigation finding:** that claim isn't real. Confirmed interactively via `hangon` (pressing ctrl-down moves the cursor from 1:1 to 2:1, viewport doesn't scroll) and by code reading — `Editor.pm`'s up/down key dispatch never branches on the ctrl modifier at all, so Ctrl+Down is currently identical to plain Down. Nothing in `qa/07_navigation.txt`'s actual `QA-NAV-010` entry ("press Down repeatedly; cursor remains visible; viewport auto-scrolls") makes this claim either — it's untested, undocumented, and unreachable from the UI, so rather than fabricate a new keybinding feature under a "test-quality only" task, rewrote the script to test what `QA-NAV-010` actually documents: pressing Down past a screen's worth of lines and asserting (a) the cursor lands on the expected line and (b) that line's content is actually rendered on screen (`View::ensure_cursor_visible`). Also found, incidentally, that `Preferences.pm`'s `scroll_margin` (default 3, "lines to keep above/below cursor") is defined but never read anywhere in `View.pm` — the vertical-scroll comment there literally says "no margins — scroll exactly when needed." Not fixed (out of scope for this task); noted here per Rule 6 for a future pass, and it also means `QA-NAV-011`'s "scroll margin" script (itself misnamed `nav_011_ctrl_right.sh`) is worth re-auditing separately. Verified: disabled the vertical-follow branch of `View::ensure_cursor_visible()` that this test's file (`.txt` extension → word-wrap-on by default in this app) actually exercises — script correctly failed (cursor's line scrolled off-screen); restored — passes. Also cut the loop's per-key sleep from the default 0.4s to the established `0.05s` convention (other scripts' repeated-key loops) — runtime dropped from 14s to 3.5s.
- **`qa/scripts/tier1/reg_070_message_persist.sh` (QA-REG-070)** — meant to verify status messages persist past 2s (`docs/UI_GUIDELINES.md:236`); computed `saved_before`/`saved_after` but never compared them, unconditionally passing either way. Fixed to two real assertions: the save message appears, and it's still present after a 2s wait with no keypress (the actual "not time-based" rule). Verified: since Zepto's main loop doesn't re-render on a pure idle timeout (no pending work), a hypothetical time-based-clear regression wouldn't even repaint the screen during the wait — so the realistic break also needs to force a render on idle tick, which any real regression would too (otherwise a user would never see it clear either). Added a synthetic 1s message expiry in `Editor.pm::show_message`/the render call plus a forced idle re-render — script correctly failed both assertions; reverted both — passes.
- **`qa/scripts/tier1/gut_002_click_minimap.sh` (QA-GUT-002)** — had a "minimap click navigated"/"minimap click executed" fallback that passed either way. Rewrote using `_handle_minimap_click`'s exact geometry (`Editor.pm` ~1820-1836/2318-2347: `text_row = y-3`, `ratio = text_row/(text_height-1)`, `target_line = round(ratio*(total_lines-1))`) to pick two clicks with **exact, deterministic** expected outcomes: row 3 (first content row) always maps to line 1; the last content row always maps to the document's last line. The bottom-row assertion can't be explained by an accidental fallthrough to ordinary text-area clicking either — with only ~20-30 lines visible from the top, an ordinary click at that row would never land on line 150 of a 150-line file. Terminal geometry (rows/cols) is read from the live screen, not hardcoded. Verified: made `_handle_minimap_click` an early no-op — script correctly failed (bottom click left cursor at line 1); restored — passes, including the pre-existing "minimap enabled" check (also made real: it now asserts the palette's `[on]` badge, not an unconditional pass).
- **`qa/scripts/tier1/prmt_006_pill_click.sh` (QA-PRMT-006)** — had a "clicking Cancel pill dismissed prompt"/"test inconclusive" fallback. Rewrote to assert Cancel's actual, specific effect (`_prompt_close_dirty_tabs`: choice `'c'` does nothing — no save, no discard, no quit; it dismisses the prompt and returns control to the editor): after the click, the prompt text is gone, the session is still alive, and — the unfakeable "verify by action" check — a typed marker string actually lands in the document (impossible if the prompt, or any stuck state, were still intercepting input). **Bug found in my own first draft, fixed before landing:** locating the "Cancel" pill's screen column with `awk`'s `index()` overshot badly (computed column 83 on a ~76-col line) because this system's `awk` computes `index()` in **bytes**, and the pill row has multi-byte UTF-8 icon glyphs before "Cancel" — switched to Perl's UTF-8-aware `index()` (`perl -CSD`) for the correct character column. **Flakiness found and fixed during the 2-3x verification runs** (Rule 5's "confirm no flakiness" step, not incidental): a bare `qa_screen` single-capture immediately after `qa_assert_expect` raced a still-in-flight render under parallel suite load and occasionally missed the prompt; switched to `qa_wait_screen` (polls), matching the pattern the helper library's own comments recommend for exactly this reason. Verified clean over 5 consecutive standalone runs after the fix. Verified non-tautological: disabled the prompt's mouse-click branch in `Editor.pm` (`elsif ($type eq 'mouse' && ... && 0)`) — script correctly failed both the dismissal and marker-input assertions (the marker's leading `C` character happened to hit the keyboard Cancel shortcut instead, a nice confirmation the test setup is sound); restored — passes.

No changelog entry — all 5 fixes above are QA-script-only changes; no product code was modified in the final (restored) state.

### ~~P2: 2 tautological unit-test subtests in `tests/completion.t`~~ FIXED
`tests/completion.t:397-425` ("Controller accept with snippets") and `:430-455` ("Controller records accepted to RecentProvider") both gated their real assertions behind `if ($ctrl->is_active())` and fell through to `pass('no completion active (acceptable)')` otherwise.

**Investigation:** confirmed both subtests' existing inputs already trigger completion 100% reliably — no input change needed, just the dead fallback removed. "Controller accept with snippets" uses a 2-char prefix `"fo"` against a `.py` file with only `SnippetProvider` registered; `SnippetProvider.pm`'s Python snippet list (`if`/`for`/`def`/`class`/`while`/`try`/`with`) has exactly one trigger starting with `"fo"` (`"for"`), so the match — and therefore `accept()`'s hashref return — is deterministic, not scoring/order-dependent. "Controller records accepted to RecentProvider" uses a 3-char prefix `"hel"` that `CrossBufferWordProvider` finds by scanning `"hello_world"` elsewhere in the same buffer — also deterministic (no scoring race, the word just needs to exist). Confirmed via a standalone script and 3x `prove tests/completion.t` runs before touching the test.

**Fix:** removed both `if ($ctrl->is_active()) { ... } else { pass(...) }` wrappers; replaced with an unconditional `ok($ctrl->is_active(), ...)` followed directly by the real assertions (previously nested inside the `if`). Also dropped the redundant `if (ref($result) eq 'HASH')`/plain-suffix branch in the snippets subtest — with `SnippetProvider` as the *only* registered provider, `accept()` can only ever return a snippet hashref.

**Verified non-tautological (Rule 5):** made `Controller::trigger()` unconditionally `return $self->dismiss()` — `prove -v tests/completion.t` now hard-fails both subtests (the first also throws a fatal `Can't use string ("") as a HASH ref` on the now-unreachable-fallback-free assertion, since there's no longer any silent path around it). Reverted — `prove tests/completion.t` passes clean, confirmed 3x. Full suite (`make test`): 41 files, 1163 tests, all pass, no noise.

No changelog entry — internal test-quality change, no product code modified in the final state.
...
### P3: `qa/scripts/tier3/` doesn't exist despite being referenced as real by docs and the Makefile — FIXED
...
**Fix:** Re-verified: `qa/scripts/` only has `tier1/` and `tier2/`; `perl qa/runner.pl --list --tier 3` still reports "No test scripts found for tier(s) 3". Searched `qa/README.md` end-to-end for any hint of what tier3 was meant to hold — it has a full `## Tier 2: LLM Vision-Judge Sweeps` section but no corresponding tier3 section or description anywhere, so there's no obvious/small legitimate content to populate it with. Went with option (a): corrected the docs and Makefile comments to describe tier3 as reserved-for-future-use rather than implying it's a populated, currently-additive coverage tier. Did not invent tier3 content or create the directory.
- Before (`qa/README.md`): `**Running it:** \`make qa-visual\` (tier 1 + 2) or \`make qa-full\` (+ tier 3). Plain \`make qa\` (tier 1 only, the default and what CI/rule-3 compliance requires) never touches any of this — no LLM dependency at all.`
- After: same first sentence (reworded "+ tier 3" to "(tiers 1,2,3)"), plus a new paragraph: `**Tier 3 is reserved, not yet populated.** \`qa-full\`/\`qa-list\` pass \`--tier 1,2,3\` to \`qa/runner.pl\` for forward-compatibility, but no scripts exist under \`qa/scripts/tier3/\` yet — \`make qa-full\` today runs identically to \`make qa-visual\`. Nothing is broken by this (the runner just reports zero tier-3 scripts found); it's a placeholder for a future coverage tier (e.g. slow/expensive checks like full-matrix performance benchmarking or cross-platform runs) that hasn't been built out. If you add the first tier-3 script, update this note.`
- Before (`Makefile`): `# All automated tiers` / `qa-full: build` ... `# List available QA scripts without running` / `qa-list:`
- After: `# All automated tiers (tier 3 is reserved for future use — no scripts exist under qa/scripts/tier3/ yet, so this currently runs identically to qa-visual; see qa/README.md)` / `qa-full: build` ... `# List available QA scripts without running (tier 3 included for forward-compatibility; see qa-full comment above)` / `qa-list:`
...
### P3: `DESIGN.md:301` — "Our build.pl is 50 lines of simple string manipulation" is stale — FIXED
...
**Fix:** Re-verified at fix time (`build.pl` is 440 lines — off by one from the audit's count, presumably a one-line drift from a concurrent change; re-checked directly rather than trusted). Replaced the stale line count and vague "simple string manipulation" description with an accurate summary of what `build.pl` actually does: walks `lib/` for `.pm` files, sorts by an explicit priority table (dependency order), applies per-file regex transforms (strip shebangs/comments, `use parent`→`@ISA`, inline `use Zepto::*`, wrap each module in its own block), embeds `docs/help/*.md` into `HelpDocs`, and appends the `main` entry point (CLI parsing, `--install`, `--version`).
- Before: `Tools like PAR or FatPacker add complexity and dependencies. Our build.pl is 50 lines of simple string manipulation. It does exactly what we need and nothing more.`
- After: `Tools like PAR or FatPacker add complexity and dependencies. Our build.pl is ~440 lines of plain Perl: it walks lib/ for .pm files, sorts them into dependency order via an explicit priority table (core modules first, Syntax::Base before other grammars, Editor.pm last), applies a handful of per-file regex transforms (strip shebangs/comments, convert use parent to @ISA, inline use Zepto::* away, wrap each module in its own { } block to avoid my-variable name clashes), embeds the docs/help/*.md files into the HelpDocs module, and appends a main entry point (CLI arg parsing, --install, --version). It does exactly what we need and nothing more — no bundler dependency, just string manipulation.`

### P3: `docs/FIND_REPLACE_SPEC.md` has three stale/wrong claims — FIXED
...
**Fix:** Re-verified all three against current code (`Editor.pm:131` `find_regex => 0`, `FindEngine.pm:33` `use_regex => 0`; `Editor.pm:2718-2723` Ctrl+R toggles `find_regex`; `Editor.pm:397-437` `_save_find_history`/`find_history` persisted via `state_store`). Also found and fixed two more instances of the same stale "Ctrl+R opens replace" claim and one wrong case-toggle key in the same file's keyboard table (line ~44 duplicated the Replace-mode claim; line ~51 said Alt+I toggles case, but code (`Editor.pm:2726-2728`) shows it's Ctrl+C) — left uncorrected these would have contradicted the primary fix two lines away in the same doc.
- Before (§Modes header): `### Replace Mode (Ctrl+R or Tab from Find)` / After: `### Replace Mode (Tab from Find)`
- Before (keyboard table): `| Ctrl+R | Open replace mode |` ... `| Alt+R (or ^R) | Toggle regex mode |` `| Alt+I (or ^I) | Toggle case sensitivity |` / After: removed the "Ctrl+R | Open replace mode" row, clarified Tab's row to `| Tab | From Find (no replace field yet): open Replace mode. Otherwise: toggle focus between find and replace fields |`, and corrected the toggle rows to `| Ctrl+R | Toggle regex mode |` / `| Ctrl+C | Toggle case sensitivity |`
- Before: `- Default: ON` (regex toggle) / After: `- Default: OFF`
- Before: `6. Search history not implemented` (in Known Issues/TODO) / After: line removed

### P3: `FEATURES.md:51`'s enumerated language list is missing 2 of the 52 languages its own header claims — FIXED
...
**Fix:** Re-verified: `ls lib/Zepto/Syntax/*.pm` (excluding `Base.pm`) gives exactly 52 files, matching the "52 Languages" header. Compared the full module list against the enumerated line and confirmed AsciiDoc and KDL were the only two missing (no other drift). Added both in alphabetical position.
- Before: `C, C++, C#, Clojure, CMake, Crontab, CSS, Diff, ... JavaScript/JSX, JSON, Kotlin, LaTeX, ...`
- After: `AsciiDoc, C, C++, C#, Clojure, CMake, Crontab, CSS, Diff, ... JavaScript/JSX, JSON, KDL, Kotlin, LaTeX, ...` (52 comma-separated entries, verified by count)

### P3: `docs/UI_GUIDELINES.md:193` directly contradicts a real, shipped, documented shortcut — FIXED
...
**Fix:** Re-verified `Ctrl+Shift+F` is genuinely shipped (`CommandRegistry.pm` `find_in_files` shortcut, `Editor.pm:1283-1295` `$shift && $ch eq 'f'` dispatch, `docs/help/tutorial.md:62`) and traced *why* it can work: `InputParser.pm`'s CSI-u/fixterms path (`ESC [ codepoint ; modifiers u`, ~line 336-342) decodes explicit modifier bits when the terminal supports that protocol (Kitty keyboard protocol, xterm `modifyOtherKeys`, etc.). But on a terminal that only sends the plain control byte (`InputParser.pm::_parse_control`, `Ctrl+letter` → ord 1-26), shift is never encoded at all — `Ctrl+Shift+F` becomes byte-for-byte identical to `Ctrl+F` and the editor's own dispatch (`Editor.pm:1274-1301`, gated on `has_modifier($event,'shift')`) falls through to plain `Find` instead. So this isn't a case where Zepto found a trick that makes the combo reliable — it's genuinely terminal-dependent, exactly the risk the original guideline warned about, it just degrades gracefully (to a still-useful action) rather than doing nothing. Chose framing (b): softened the absolute rule into risk-flagged guidance rather than adding a false "this one is safe" exception.
- Before: `- Do not depend on \`Shift+letter\` or \`Ctrl+Shift+letter\`; terminals cannot distinguish these reliably.`
- After: `- Treat \`Shift+letter\` or \`Ctrl+Shift+letter\` as risky, not off-limits: whether the terminal can distinguish them depends on the terminal supporting an extended keyboard protocol (CSI-u/"fixterms", the Kitty keyboard protocol, or xterm \`modifyOtherKeys\`) that encodes modifier bits explicitly. \`InputParser.pm\`'s CSI-u path (\`ESC [ codepoint ; modifiers u\`) decodes these correctly when present. Without it, a terminal sends the same plain control byte for \`Ctrl+F\` and \`Ctrl+Shift+F\` (shift is not encoded in a bare control byte, \`InputParser.pm::_parse_control\`), and the two become indistinguishable — Zepto's own \`find_in_files\` (bound to \`Ctrl+Shift+F\`) silently degrades to plain \`Find\` on such terminals rather than erroring. Only reach for a \`Ctrl+Shift+letter\` binding when: (a) the feature is important enough to justify graceful degradation to a different, still-reasonable action on older terminals, and (b) that degraded behavior is itself acceptable (not a silent no-op or something surprising). Prefer an unambiguous binding when one is available.`

### P3: `DESIGN.md` labels `Renderer` and `WrapMap` "Pure function" — both claims are false — FIXED
...
**Fix (docs only — code-level refactor explicitly out of scope):** Re-verified both claims directly: `Renderer.pm` has `%_cdw_cache` (line 139), `%_file_exists_cache` (1429), `%_image_dims_cache` (1444), and a closure-scoped tab-bar cache (`_tab_bar_cache_get`/`_set`, ~345-358); `WrapMap.pm` calls `Zepto::Renderer::_expand_tabs()`/`_char_to_visual_col()`/`visual_to_char_col()` at lines 268, 288, 445, 483, the first two being underscore-prefixed (conventionally private). Corrected the `DESIGN.md` module table and prose to describe both accurately without implying they're pure. Did **not** touch the actual code — no public API was added to `Renderer.pm`, `WrapMap.pm` still reaches into the private helpers; that refactor remains a separate, larger, riskier task.
- Before (table): `| **Renderer** | Converts editor state to ANSI escape sequences | Pure function |`
- After: `| **Renderer** | Converts editor state to ANSI escape sequences | State in → string out, with internal memoization caches (char-width, file-exists, image-dims, tab-bar geometry) |`
- Before (table): `| **WrapMap** | Word wrap computation, visual row mapping | Pure function |`
- After: `| **WrapMap** | Word wrap computation, visual row mapping | State in → wrap map out; calls into Renderer's private \`_\`-prefixed tab/column helpers directly (accepted coupling, not a public API) |`
- Before (prose): `The key architectural enabler: keeping Renderer pure (state in → string out) means rendering can be tested without a terminal.`
- After: `The key architectural enabler: Renderer's core contract is state in → string out (it holds no reference to the terminal or editor object, just internal memoization caches keyed on its inputs), which means rendering can be tested without a terminal.`
...
### ~~P3: Kitty/fixterms CSI-u modified special keys (Ctrl+Enter, etc.) are silently and permanently dropped~~ FIXED (2026-08-30)
`InputParser.pm:337-342` only handled CSI-u codepoints in the printable range (32-126); codepoints for Enter (13)/Tab (9)/Backspace (8)/Escape (27) sent with a modifier via the Kitty keyboard protocol fell through to "Unknown CSI sequence" → `EVT_NONE`. An earlier fix (already tracked in this file, "Out-of-range CSI-u does not stall following events") resolved the *stall* this used to cause for the *following* keystroke, but didn't recover the dropped codepoint itself — on a Kitty-protocol terminal, shortcuts like `Ctrl+Enter` simply did nothing, with no code path that could make them work.

**Root cause:** the CSI-u branch (`ESC [ codepoint ; modifiers u`) only had a case for `32 <= codepoint < 127` (mapped to `_make_char_event`). Enter/Tab/Backspace/Escape are control-character codepoints outside that range, so they fell straight through the `if` with no `else`, hit the function's final `return { type => EVT_NONE }`, and were discarded — even though `_parse_control` (the plain, non-CSI-u path) already had an identical mapping for the exact same codepoints one section below in the same file.

**Fix:** Added four more codepoint checks inside the same CSI-u branch, reusing the existing `_make_key_event($key, $modifiers)` helper — the same helper every other CSI-u special key (arrows, Home/End, F-keys) already uses to attach modifiers to a named key event, so Ctrl+Enter now produces `{ type => 'key', key => 'enter', modifiers => ['ctrl'] }`, structurally identical to how e.g. Ctrl+Right already looks, just with a different `key` value. Mapping: `13 → KEY_ENTER`, `9 → KEY_TAB`, `8` and `127 → KEY_BACKSPACE` (127/DEL included because `_parse_control`'s plain-backspace path already treats DEL and BS as equivalent — matched here for consistency), `27 → KEY_ESCAPE`. Scoped to `InputParser.pm` only — no existing command in `CommandRegistry.pm`/`Editor.pm` binds Ctrl+Enter or similar today, so the value of this fix is that the keystroke now produces a real, potentially-bindable event instead of vanishing; no new keybindings were added.

**Tests:** `tests/input_parser.t` — reproduced first (confirmed the new assertions fail against the pre-fix code), then fixed, then reconfirmed passing. Added a new subtest covering all four codepoints (8/9/13/27) plus the 127/DEL alias, each asserting a real `key`-type event with the correct `key` and `['ctrl']` modifier comes back, not `EVT_NONE`. The pre-existing "Out-of-range CSI-u does not stall following events" subtest was updated to use codepoint 3 (Ctrl+C/ETX, which still has no mapping) since codepoint 13 — its original example — is no longer an out-of-range case after this fix. `make test` (1164 tests) and `make check` clean.

**Interactive verification:** achieved for real, not skipped. No actual Kitty-protocol terminal was needed — `InputParser` parses whatever raw bytes arrive on stdin regardless of what the terminal advertises, so `hangon send --stdin` was used to inject literal CSI-u byte sequences (`ESC [ 8/9/13/27 ; 5 u`) directly into a running `./zepto` session's pty. Confirmed via the rendered screen: Ctrl+Backspace deleted the last typed character (cursor 1:4 → 1:3, "abc" → "ab"), Ctrl+Enter split the line (cursor → 2:1), Ctrl+Tab advanced the cursor past column 1, and Ctrl+Escape closed an open command palette (`╭` dialog border disappeared) — reverting the fix and rebuilding reproduced all four failures again, confirming both directions.

**QA:** `QA-REG-202` (`qa/scripts/tier1/reg_202_csiu_special_keys.sh` + `qa/40_regression_bugs.txt`) automates the same four-key interactive repro above using the status bar's `line:col` indicator and the palette's `╭` dialog-border glyph as robust assertions (the gutter cursor glyph itself was found to be unreliable as a test signal — it varies between a plain `>` and a nerd-font powerline glyph depending on terminal-capability detection, unrelated to this bug). Verified failing against the pre-fix binary (5/10 assertions fail, exactly the four CSI-u-mapped keys) and passing against the fix (10/10).
